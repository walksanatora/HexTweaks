    ...
    init {
        ...
        register(modloc("entity"), HexIotaTypes.ENTITY, object : IotaSerde<EntityIota> {
            ...
            override fun deserialize(value: Map<*, *>, world: Level): EntityIota? {
                if (net.walksanator.hextweaks.HexTweaks.getCONFIG().allowUnsafeDeserialization == SecurityLevel.RESTRICT) {return null}
                val uuid = UUID.fromString(value["uuid"] as? String?: return null)
                if (net.walksanator.hextweaks.HexTweaks.getCONFIG().allowUnsafeDeserialization == SecurityLevel.TRUENAME) {
...
private fun modloc(string: String) = ResourceLocation(net.walksanator.hextweaks.HexTweaks.MOD_ID, string)
